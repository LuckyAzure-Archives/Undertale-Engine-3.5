...
func _physics_process(_delta):
	...
	if currentscene.get_node("Soul/Soul").Damage == 1:
		currentscene.get_node("SoundChannels/SFXSoulChannel1").stream = load("res://iso/battle-engine/sounds/damage.wav")
		...
		if currentscene.HPTYPE == "":
			...
			if currentscene.HP < 1:
				...
				changescene = get_tree().change_scene("res://src/scenes/game-over/Game Over.tscn")
		else:
